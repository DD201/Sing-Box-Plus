...
sbp_pm_refresh() {
  case "$PM" in
    apt)
      ...
      [[ -f /etc/apt/sources.list ]] && sed -i -E 's#^(deb\s+https?://security\.debian\.org)(/debian-security)?\s+bullseye/updates(.*)$#\1/debian-security bullseye-security\3#' /etc/apt/sources.list 2>/dev/null || true
...
REALITY_SERVER=${REALITY_SERVER:-images.apple.com}
...
write_config(){
  ...
  '
  def inbound_vless($port): {type:"vless", listen:"::", listen_port:$port, users:[{uuid:$UID}], tls:{enabled:true, server_names:["images.apple.com","www.apple.com","music.apple.com","www.cloudflare.com"], reality:{enabled:true, handshake:{server:$RS, server_port:$RSP}, private_key:$RPR, short_id:$SID}, alpn:["h2"]}, transport:{type:"tcp"}};
  def inbound_vless_flow($port): {type:"vless", listen:"::", listen_port:$port, users:[{uuid:$UID, flow:"xtls-rprx-vision"}], tls:{enabled:true, server_names:["images.apple.com","www.apple.com","music.apple.com","www.cloudflare.com"], reality:{enabled:true, handshake:{server:$RS, server_port:$RSP}, private_key:$RPR, short_id:$SID}, alpn:["h2"]}, transport:{type:"tcp"}};
  def inbound_trojan($port): {type:"trojan", listen:"::", listen_port:$port, users:[{password:$UID}], tls:{enabled:true, server_names:["images.apple.com","www.apple.com","music.apple.com","www.cloudflare.com"], reality:{enabled:true, handshake:{server:$RS, server_port:$RSP}, private_key:$RPR, short_id:$SID}, alpn:["h2"]}, transport:{type:"tcp"}};
  ...
  def inbound_hy2_obfs($port): {type:"hysteria2", listen:"::", listen_port:$port, users:[{name:"hy2", password:$HY22}], obfs:{type:"salamander", password:$HY2O}, tls:{enabled:true, certificate_path:$CRT, key_path:$KEY}};
